...
- Release 提供时，会把 `codex-code-mode-host` 安装到 `codex` 同一目录
...
对于当前官方版本，`codexi` 会把 `codex` 和 `codex-code-mode-host` 直接安装到配置的二进制目录。没有 code-mode host 资产的旧版本仍只安装 `codex`。
...
- 提示 `failed to spawn code-mode host`：先升级 `codexi`，再执行 `codexi update`；缺失的 host 会被安装到 `codex` 同一目录。
